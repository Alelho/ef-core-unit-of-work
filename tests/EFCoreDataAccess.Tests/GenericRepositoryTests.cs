...
namespace EFCoreDataAccess.Tests
{
	...
	public class GenericRepositoryTests
	{
		...
		[Fact]
		public void LastOrDefault_ShouldReturnTheLastEntityWithYourChildren_GivenTableWithEntities()
		{
			// Arrange
			using var scope = _databaseFixture.ServiceProvider.CreateScope();
			var uow = scope.ServiceProvider.GetService<IUnitOfWork<EmployeeDbContext>>();
			...
			// Act
			var result = companyRepository.LastOrDefault(c => c.Id > 0, c => c.Id, includeQuery);
		...
		[Fact]
		public async Task FirstOrDefaultAsync_ShouldReturnAnEntityWithYourChildren_GivenValidEntityId()
		{
			// Arrange
			using var scope = _databaseFixture.ServiceProvider.CreateScope();
			var uow = scope.ServiceProvider.GetService<IUnitOfWork<EmployeeDbContext>>();
			...
			// Act
			var entity = await companyRepository.FirstOrDefaultAsync(c => c.Id == validEntity, includeQuery);
		...
		[Fact]
		public void SingleOrDefault_ShouldReturnAnEntityWithYourChildren_GivenValidEntityId()
		{
			// Arrange
			using var scope = _databaseFixture.ServiceProvider.CreateScope();
			var uow = scope.ServiceProvider.GetService<IUnitOfWork<EmployeeDbContext>>();
			...
			// Act
			var result = companyRepository.SingleOrDefault(c => c.Id == validEntity, includeQuery);

			// Assert
			using (new AssertionScope())
			{
				result.Should().NotBeNull();
				result.Id.Should().Be(validEntity);
				result.Address.Should().NotBeNull();
				result.Employees.Should().NotBeNullOrEmpty();
				result.Employees.Select(o => o.EmployeeEarnings).Should().NotBeNullOrEmpty();
			}
		...
		[Fact]
		public async Task LastOrDefaultAsync_ShouldReturnTheLastEntityWithYourChildren_GivenTableWithEntities()
		{
			// Arrange
			using var scope = _databaseFixture.ServiceProvider.CreateScope();
			var uow = scope.ServiceProvider.GetService<IUnitOfWork<EmployeeDbContext>>();
			...
			// Act
			var result = await companyRepository.LastOrDefaultAsync(c => c.Id > 0, c => c.Id, includeQuery);
